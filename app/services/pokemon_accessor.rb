...
class PokemonAccessor
    ...
    def call
        (1..6).each do |pokemon_number|
            uri = URI("https://pokeapi.co/api/v2/pokemon/#{pokemon_number}")
    ...
    def find_generation(pokemon)
        # Opens the URL and reads it
        html_doc = Nokogiri::HTML(open("https://pokemondb.net/pokedex/#{pokemon}").read)

        # Searches the URL for the generation in which that Pokémon was introduced
